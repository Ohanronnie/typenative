...
| Uppercase `String` and `.toString()`                                 | owned `string`, borrowed `&str`, `string.from(...)`, and contextual literal-to-owned conversion                                                                     | type resolution, body checking, ownership, HIR/MIR conversion, LLVM lowering, Node mapping                                        | `std/string.tn`, `std/env.tn`, `std/path.tn`, validation, documentation examples | `TYPE_OBSOLETE_PUBLIC_TYPE`; `obsolete-collections.tn`; canonical string native validation                       |
